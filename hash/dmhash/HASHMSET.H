...
// allocation/deallocation
    ...
    hash_multiset(size_type table_size = 10007, float maxLF = 1.1, 
      const HashFunction& hf = HashFunction(),
      const KeyEqual& equal = KeyEqual()) 
      : ht(table_size, maxLF, hf, equal, true) {}

    hash_multiset(const value_type* first, const value_type* last,
        size_type table_size = 10007, float maxLF = 1.1, 
        const HashFunction& hf = HashFunction(),
        const KeyEqual& equal = KeyEqual()) 
        : ht(first, last, table_size, maxLF, hf, equal, true) {}

    friend void destroy(hash_multiset<Key, HashFunction, KeyEqual>* p) {
      p->~hash_multiset();
    }
...
// copying, assignment, swap
    ...
    void swap(hash_multiset<Key, HashFunction, KeyEqual>& x) { ht.swap(x.ht); }
...
// search operations:
    ...
    friend ostream& 
    operator<<(ostream& o, 
    const hash_multiset<Key, HashFunction, KeyEqual>& m) {
         o << m.ht;
        return o;
    }
...
template <class Key, class HashFunction, class KeyEqual>
inline bool operator==(const hash_multiset<Key, HashFunction, KeyEqual>& x, 
